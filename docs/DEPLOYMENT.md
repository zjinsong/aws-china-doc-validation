...
- 用于只读验证的 EC2 IAM Role。按实际文档功能授予最小的 `List*`/`Describe*` 权限；首批专用探针需要 `ec2:DescribeInstanceTypeOfferings` 和 `dynamodb:ListGlobalTables`。不要在生产环境使用管理员权限。
...
- IAM Role 遵循最小权限，仅允许文档功能所需的 List/Describe 操作；禁止 Create/Update/Put/Delete。
...
- 先用 `query_knowledge_base` 和 `verify_feature` 验证 MCP 客户端连通性，再用 `verify_document_feature` 对测试文档执行只读探针。

`verify_document_feature` 不返回或持久化 List/Describe 的账户资源响应，只记录 API 名称、HTTP 状态、错误码和非账户级 feature 证据。`AccessDenied`、凭证缺失或需要资源名称时返回 `unknown`，不会被解释为功能不可用。
