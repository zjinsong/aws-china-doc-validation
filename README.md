...
```
┌─────────────────────────────────────────────────────────┐
│              NWCD AI Chat (LibreChat)                    │
│    ┌─────────────────────────────────────────┐          │
│    │  智能体: China Region Checker           │          │
│    │  - 模型: Qwen3-235B-VL                  │          │
│    │  - MCP: 通过 SSE URL 连接               │          │
│    └──────────────────┬──────────────────────┘          │
└───────────────────────┼─────────────────────────────────┘
...
| **LLM 推理** | 理解用户问题、决定验证策略、生成文档审计报告 | Qwen3-235B-VL (NWCD AI Chat) |
| **Tool Use** | 自动调用 AWS API 验证 feature 可用性 | boto3 + 中国区测试账号 |
| **知识积累** | 验证结果自动回写知识库，形成数据飞轮 | SQLite 持久化 |

### 关键技术点

- **MCP 远程服务**：通过 SSE transport 暴露 MCP Server，NWCD AI Chat 通过 URL 连接
...
3. **NWCD AI Chat 配置**：
   - 在 NWCD AI Chat 中创建智能体
   - 添加 MCP 服务器，填入 Server URL（如 `https://<your-ec2-ip>:8000/sse`）
   - Transport 选择 SSE

### 使用

在 NWCD AI Chat 中与智能体对话：
