...
| AI 能力 | 具体应用 | 实现方式 |
|---------|---------|---------|
| **MCP (Model Context Protocol)** | 暴露 5 个 tools 供智能体调用 | FastMCP + SSE transport |
...
- **服务端点验证**：`verify_feature` 仅检查 Botocore 的 `aws-cn` 服务区域目录，不代表具体 feature 可用
- **文档驱动验证**：`verify_document_feature` 从中国区文档提取 List/Describe API，只执行无需账户资源标识的安全探针
...
`verify_document_feature(documentation_url, service, feature, region)` 会读取指定中国区文档，发现相关的 List/Describe API，并使用 AWS 中国区凭证调用目标区域端点。通用探针只确认 API 可调用；EC2 实例系列会通过 `DescribeInstanceTypeOfferings` 验证实际供应，DynamoDB Global Tables 会通过 `ListGlobalTables` 验证 API 可达。工具不会调用 Create、Update、Put 或 Delete API，也不会保存 API 返回的账户资源列表。
